	...
	<div class="row justify-content-center">
		<div class="col-md-12">
			<div class="card">
				<div class="card-header">
					<div style="display: flex; justify-content: space-between; align-items: center;">
						<div class="float-left">
							...
							Listado de especiales</h4>
						</div>
						@if (session()->has('message'))
						<div wire:poll.4s class="btn btn-sm btn-success" style="margin-top:0px; margin-bottom:0px;"> {{ session('message') }} </div>
						@endif
						<div>
							<input wire:model='keyWord' type="text" class="form-control" name="search" id="search" placeholder="Search Especiales">
						</div>
				...
				<div class="card-body">
						@include('livewire.especiales.modals')
				<div class="table-responsive">
					<table class="table table-bordered table-sm">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{ $row->actor }}</td>
								<td>{{ $row->entidad }}</td>
								<td>{{ $row->trabajador_fallecido }}</td>
								...
								<td width="90">
									<div class="dropdown">
										...
										<ul class="dropdown-menu">
											...
											<li><a class="dropdown-item" onclick="confirm('Confirm Delete Especiale id {{$row->id}}? \nDeleted Especiales cannot be recovered!')||event.stopImmediatePropagation()" wire:click="destroy({{$row->id}})"><i class="fa fa-trash"></i> Eliminar </a></li>  
										</ul>
									</div>								
								</td>
							</tr>
					...
					<div class="float-end">{{ $especiales->links() }}</div>
					</div>
				</div>
			</div>
		</div>
